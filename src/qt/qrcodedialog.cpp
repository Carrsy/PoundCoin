...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("PND")
    updateDisplayUnit();
}
...
{
    QString ret = QString("poundcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PND unit input in lnReqAmount, we generate the URI with PND as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::PND, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
